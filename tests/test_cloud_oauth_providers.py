...
@respx.mock
def test_cloud_oauth_provider_secret_reference_is_write_only() -> None:
    ...
    response = client.cloud_oauth_providers.update("github", client_id="client", client_secret_ref=ref,
                                                    enabled=True, idempotency_key="oauth-1")
    assert route.calls.last.request.headers["Idempotency-Key"] == "oauth-1"
    assert __import__("json").loads(route.calls.last.request.content)["client_secret_ref"] == ref
    assert "client_secret_ref" not in response
