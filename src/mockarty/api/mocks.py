...
class MockAPI(SyncAPIBase):
    ...
    def batch_create(
        self, mocks: list[Mock | dict[str, Any]]
    ) -> list[SaveMockResponse]:
    ...
    def restore_version(self, mock_id: str, version: int) -> None:
        ...
        self._request("POST", f"/api/v1/mocks/{mock_id}/versions/{version}/restore")

    def delete_logs(self, mock_id: str) -> None:
        """Delete all request logs for a mock."""
        self._request("DELETE", f"/api/v1/mocks/{mock_id}/logs")

    def copy_to_namespace(self, ids: list[str], target_namespace: str) -> None:
...
class AsyncMockAPI(AsyncAPIBase):
    ...
    async def delete_logs(self, mock_id: str) -> None:
        """Delete all request logs for a mock."""
        await self._request("DELETE", f"/api/v1/mocks/{mock_id}/logs")

    async def copy_to_namespace(self, ids: list[str], target_namespace: str) -> None:
        """Copy mocks to another namespace."""
        await self._request(
            "POST",
            "/api/v1/mocks/copy-to-namespace",
