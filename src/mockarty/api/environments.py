...
class EnvironmentAPI(SyncAPIBase):
    ...
    def get(self, env_id: str) -> dict[str, Any]:
        ...
        resp = self._request("GET", f"/api/v1/api-tester/environments/{env_id}")
        return resp.json()

    def create(self, env: dict[str, Any]) -> dict[str, Any]:
        """Create a new environment."""
        resp = self._request("POST", "/api/v1/api-tester/environments", json=env)
        return resp.json()
    ...
    def delete(self, env_id: str) -> None:
        ...
        self._request("DELETE", f"/api/v1/api-tester/environments/{env_id}")

    def activate(self, env_id: str) -> None:
        """Activate an environment."""
        self._request("POST", f"/api/v1/api-tester/environments/{env_id}/activate")
...
class AsyncEnvironmentAPI(AsyncAPIBase):
    ...
    async def get_active(self) -> dict[str, Any]:
        """Get the currently active environment."""
        resp = await self._request("GET", "/api/v1/api-tester/environments/active")
        return resp.json()

    async def get(self, env_id: str) -> dict[str, Any]:
        """Get an environment by ID."""
        resp = await self._request("GET", f"/api/v1/api-tester/environments/{env_id}")
        return resp.json()

    async def create(self, env: dict[str, Any]) -> dict[str, Any]:
        """Create a new environment."""
        resp = await self._request("POST", "/api/v1/api-tester/environments", json=env)
        return resp.json()

    async def update(self, env_id: str, env: dict[str, Any]) -> dict[str, Any]:
        """Update an existing environment."""
    ...
    async def delete(self, env_id: str) -> None:
        """Delete an environment."""
        await self._request("DELETE", f"/api/v1/api-tester/environments/{env_id}")
