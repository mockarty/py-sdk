...
Run:
    MOCKARTY_SERVER=http://localhost:5770 MOCKARTY_API_KEY=mk_... python external_runs_lifecycle.py
"""
...
def main() -> None:
    ...
    run = er.start_run({"name": "checkout smoke", "framework": "custom", "full_name": "suites.checkout.smoke"})
    run_id = run["id"]
    print(f"started run {run_id}")

    for step in [
        {"step_key": "login", "name": "log in", "status": "passed", "duration_ms": 120},
        {"step_key": "cart", "name": "add to cart", "status": "passed", "duration_ms": 80},
        {"step_key": "pay", "name": "pay", "status": "failed", "message": "gateway 500", "duration_ms": 210},
    ]:
        er.append_steps(run_id, [step])

    fin = er.finish_run(run_id, "failed", summary="payment gateway returned 500")
    print(f"finished: status={fin.get('status')} case={fin.get('resolved_case_id')} run={fin.get('resolved_run_id')}")
